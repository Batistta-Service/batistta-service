...
function exportOrdersXLSX(clients, orders) {
  ...
  const rows = orders.map((o) => {
    ...
    return {
      ...
      Cliente: c ? c.name : "",
      "Teléfono": o.phone,
      "Tipo de máquina": o.machineType,
      Modelo: o.machineModel,
      Problema: o.issue,
      Estado: statusLabel(o.status),
      Contactado: o.contacted ? "Sí" : "No",
      "Fecha recibido": o.dateReceived,
      Notas: notes,
    };
  });
  const ws = XLSX.utils.json_to_sheet(rows);
  ws["!cols"] = [{ wch: 12 }, { wch: 22 }, { wch: 12 }, { wch: 14 }, { wch: 16 }, { wch: 30 }, { wch: 16 }, { wch: 11 }, { wch: 13 }, { wch: 40 }];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Reparaciones");
  XLSX.writeFile(wb, `batistta-service-reparaciones-${new Date().toISOString().slice(0, 10)}.xlsx`);
}
  ...
  return (
    <div className="min-h-screen bg-[#17191B]" style={{ fontFamily: "Inter" }}>
      ...
      <header className="border-b border-[#2E3234] px-4 sm:px-6 py-3.5 sticky top-0 bg-[#17191B]/95 backdrop-blur">
        ...
        />
        <button onClick={() => exportOrdersXLSX(clients, orders)} className="w-full bg-[#E8A23D]/10 border border-[#E8A23D]/40 text-[#E8A23D] text-xs font-medium py-2 rounded-md">

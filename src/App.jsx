...
const seedOrders = [
  { id: "o1", ticket: "OT-2026-001", phone: "5550142", machineType: "Horno", machineModel: "Rotativo RX-200", issue: "No alcanza temperatura, tarda demasiado en calentar.", status: "diagnostico", dateReceived: "2026-08-18", notes: [{ date: "2026-08-19", text: "Se revisó resistencia principal, en espera de multímetro." }] },
  { id: "o2", ticket: "OT-2026-002", phone: "5550198", machineType: "Amasadora", machineModel: "Espiral AE-40", issue: "Ruido metálico fuerte al amasar en velocidad 2.", status: "espera", dateReceived: "2026-08-15", notes: [{ date: "2026-08-16", text: "Rodamiento del eje dañado, se pidió repuesto al proveedor." }] },
];
  ...
  return (
    <div className="min-h-screen bg-[#17191B] flex items-center justify-center p-4" style={{ fontFamily: "Inter" }}>
      ...
      >
        ...
        <label className="text-[11px] uppercase tracking-wide text-[#7A7D7F] font-medium mb-1.5 block">Tu número de teléfono</label>
        <input
          value={phone}
        ...
        <div className="mt-5 pt-5 border-t border-[#2E3234]">
          ...
          <div className="mt-3.5 pt-3.5 border-t border-[#2E3234]">
            {staffMode ? (
              <>
                ...
                <button type="button" onClick={onViewAllClients} className="w-full bg-[#E8A23D]/10 border border-[#E8A23D]/40 text-[#E8A23D] text-xs font-medium py-2 rounded-md">
              ...
              <div>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#24282B] border border-[#33383a] rounded-md px-3 py-1.5 text-xs text-[#EDE9E2] placeholder:text-[#5F6264] focus:outline-none focus:border-[#E8A23D]"
                  />
                  <button type="button" onClick={submitPin} className="bg-[#E8A23D] text-[#1A1D1F] text-xs font-medium px-3 py-1.5 rounded-md">Entrar</button>
                </div>
                {pinError && <p className="text-[#C1594A] text-[11px] mt-1.5">Clave incorrecta.</p>}
              </div>
            ) : (
              <button type="button" onClick={() => setShowPinBox(true)} className="w-full text-[11px] text-[#7A7D7F] hover:text-[#EDE9E2] py-1.5">Activar modo taller</button>
            )}
          </div>
        </div>
      </div>
...
function OrderRow({ order, onStatusChange, onAddNote, staffMode }) {
  ...
  return (
    <div className="bg-[#1F2224] border border-[#2E3234] rounded-md overflow-hidden">
...
function ClientListView({ clients, orders, onSelectClient, onBack }) {
  ...
  const rows = clients
    .map((c) => {
      ...
      return { ...c, count: myOrders.length, active, latestStatus: latest ? STATUSES.find((s) => s.id === latest.status) : null };
    })
    ...
    <div className="min-h-screen bg-[#17191B]" style={{ fontFamily: "Inter" }}>
      ...
      <header className="border-b border-[#2E3234] px-4 sm:px-6 py-3.5 sticky top-0 bg-[#17191B]/95 backdrop-blur">
        ...
        <input
          ...
          className="w-full bg-[#24282B] border border-[#33383a] rounded-md px-3 py-2 text-sm text-[#EDE9E2] placeholder:text-[#5F6264] focus:outline-none focus:border-[#E8A23D]"
        />
      </header>
      ...
      <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-2">
        ...
        {rows.map((c) => (
          <button
            ...
            className="w-full text-left bg-[#1F2224] border border-[#2E3234] hover:border-[#E8A23D] rounded-md p-3.5 flex items-center justify-between transition-colors"
          >
...
function RepairsView({ client, orders, onStatusChange, onAddNote, onNewOrder, onChangeNumber, staffMode }) {
  ...
  return (
    <div className="min-h-screen bg-[#17191B]" style={{ fontFamily: "Inter" }}>
      ...
      <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-2.5">
        ...
        {myOrders.map((o) => (
          <OrderRow key={o.id} order={o} onStatusChange={onStatusChange} onAddNote={onAddNote} staffMode={staffMode} />
        ))}
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      try {
        let c, o;
        try { c = await window.storage.get("clients", true); } catch { c = null; }
        try { o = await window.storage.get("orders", true); } catch { o = null; }
        setClients(c ? JSON.parse(c.value) : seedClients);
        setOrders(o ? JSON.parse(o.value) : seedOrders);
        if (!c) await window.storage.set("clients", JSON.stringify(seedClients), true);
        if (!o) await window.storage.set("orders", JSON.stringify(seedOrders), true);
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    (async () => {
      try { await window.storage.set("clients", JSON.stringify(clients), true); setSaveError(false); }
  ...
  useEffect(() => {
    if (!loaded.current) return;
    (async () => {
      try { await window.storage.set("orders", JSON.stringify(orders), true); setSaveError(false); }
      catch (err) { console.error(err); setSaveError(true); }
    })();
  }, [orders]);
  ...
  return (
    <>
      {saveError && (
        <div className="fixed top-3 right-3 z-50 flex items-center gap-1.5 bg-[#C1594A] text-white text-[11px] px-3 py-1.5 rounded-md shadow-lg">
          <AlertCircle size={12} /> No se pudo guardar el último cambio
        </div>
      )}
      <RepairsView
        client={client}

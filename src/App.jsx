...
function PhoneGate({ onSubmit, error, staffMode, onStaffLogin, onStaffLogout }) {
      ...
      >
        ...
        <div className="mt-5 pt-5 border-t border-[#2E3234]">
          ...
          <div className="mt-3.5 pt-3.5 border-t border-[#2E3234]">
            {staffMode ? (
              <button type="button" onClick={onStaffLogout} className="w-full flex items-center justify-center gap-1.5 text-[11px] text-[#7FA872] border border-[#2E3234] py-2 rounded-md"><Check size={12} /> Modo taller activo</button>
            ) : showPinBox ? (
              <div>
...
export default function App() {
  ...
  const [screen, setScreen] = useState("gate"); // gate | intake | repairs
  ...
  if (screen === "gate") {
    return <PhoneGate onSubmit={handlePhoneSubmit} error={phoneError} staffMode={staffMode} onStaffLogin={handleStaffLogin} onStaffLogout={handleStaffLogout} />;
  }
